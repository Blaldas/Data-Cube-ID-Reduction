...
public class DataCube {
    ...
    public DIntArray pointQuerySeach(int[][] query) {
        ...
        for (int i = 0; i < query.length; i++) {
            if (query[i][0] != -88 && query[i][0] != -99) {
                DIntArray secundary;
                if (query[i].length == 1)
                    secundary = shellFragmentList[i].getTidsListFromValue(query[i][0]);
                else
    ...
    public void getSubCube(int[][] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            if (values[i].length == 1 && (values[i][0] == -99 || values[i][0] == -88))
                str.append('*').append(" ");
            else if (values[i].length > 1) {
                str.append(Arrays.toString(values[i])).append(" ");
            } else
                str.append(values[i][0]).append(" ");
        }
    ...
    private void showQueryDataCube(int[][] qValues, int[] mapeamentoDimInq, ShellFragment[] subCube) {
        ...
        do {
            ...
            for (int i = 0; i < qValues.length; i++)
                arrayQueriesEResultados[rounds][i] = values[i][0];
